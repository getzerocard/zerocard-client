...
export const UsernameModalProvider: React.FC<UsernameModalProviderProps> = ({
  ...
  const setUsername = async (username: string) => {
    try {
      ...
      // Also save the actual username if needed elsewhere, e.g., await AsyncStorage.setItem('username', username);
      setHasSetUsername(true);
      hideUsernameModal();
    } catch (error) {
  ...
  return (
    <UsernameModalContext.Provider
      ...
      {/* Conditionally render the correct modal based on Platform */}
      {Platform.OS === 'ios' ? (
        <UsernameModal
          visible={isVisible}
